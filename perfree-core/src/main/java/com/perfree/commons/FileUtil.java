...
public class FileUtil {
    ...
    public static String uploadMultiFile(MultipartFile multiFile, String dirPath,String categoryPath) throws Exception {
        try{
            ...
            // 文件存储的子目录
            String uploadPath = SystemEnum.SYS_SEPARATOR.getValue() + categoryPath + SystemEnum.SYS_SEPARATOR.getValue() + formatter.format(new Date()) + SystemEnum.SYS_SEPARATOR.getValue();
            // 组装保存的目录
            dirPath += uploadPath;
    ...
    public static String genUploadPath(MultipartFile file, String category) throws Exception {
        ...
        String filename = StringUtil.getUUID() + suffix;
        SimpleDateFormat formatter = new SimpleDateFormat("yyyyMMdd");
        return category +  SystemEnum.SYS_SEPARATOR.getValue() + formatter.format(new Date()) + SystemEnum.SYS_SEPARATOR.getValue() + filename;
    }
